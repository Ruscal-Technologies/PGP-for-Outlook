...
function installStubs({ bodyText = '', attachments = [] } = {}) {
  ...
  return { encryptBtn, decryptBtn, statusEl, getAsync, setAsync, getAttachmentsAsync };
}
...
async function refreshComposeButtons() {
  ...
  el('btn-decrypt').classList.toggle('pgp-hidden', !isEncrypted);
  el('btn-encrypt').classList.toggle('pgp-hidden', isEncrypted);
}
```
...
```javascript
import { getSessionKey, cacheSessionKey, clearSessionKey } from '../web/js/pgp/session-cache.js';
