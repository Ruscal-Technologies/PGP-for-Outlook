...
//   - Once set up, it only listens for a bounded amount of time -- a reply
//     window that was never the target of an actual handoff (e.g. the user
//     used Outlook's own Reply button on an encrypted-but-undecrypted
...
// or item.inReplyTo as a fallback) rather than fixed, so a listener needs to
// already know which conversation/message to target — see
// reply-handoff-channel.js's docblock for why this isn't a secrecy boundary
// on its own.
//
// The actual listener/splice logic lives in
// web/js/pgp/reply-handoff-runtime-core.js, shared with web/ReplyHandoffPane.js
// -- a dedicated minimal pane opened automatically (via an InsightMessage
// notification) on hosts where BroadcastChannel isn't available in the
// OnNewMessageCompose event handler's own runtime (see #22). This pane's own
// listener, armed here whenever the user manually opens this pane, is one of
// two ways the handoff gets armed now (the other being ReplyHandoffPane.js)
// -- kept exactly as before as a fallback: hosts below Mailbox 1.10 (no
// event-based activation at all), tenants that haven't yet re-approved the
// updated manifest, or simply a user who opens this pane manually before the
// automatic path completes.
export async function setupReplyHandoffListener(has110 = _has110, has114 = _has114) {
  await armReplyHandoffListener({ has110, has114, onStatus: showStatus });
}
