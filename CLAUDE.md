...
**Normal-sized messages** (the common case): `MessageRead.js`'s reply buttons call `Office.context.mailbox.displayNewMessageFormAsync`/`displayNewMessageForm` with a quoted copy of the decrypted body as `formData.htmlBody`. Office.js caps `htmlBody` at 32 KB (32,768 characters) — confirmed to be the exact same cap on `Office.ReplyFormData.htmlBody` (used by `displayReplyForm`/`displayReplyAllForm`), so switching APIs alone would not raise it — and Outlook Classic enforces this synchronously, throwing `Sys.ArgumentOutOfRangeException` if exceeded. `buildQuotedReplyHtml()` (exported from `MessageRead.js`) keeps the quote under that limit: it returns the fully-formatted HTML quote unchanged when it fits; if the formatted HTML would exceed the limit it falls back to a plain-text quote (never truncates raw HTML, to avoid emitting unbalanced tags); if even that is too large, it truncates the text and appends a visible "[Original message truncated...]" notice. Content formatting itself (HTML-vs-plain-text rendering, `<head><style>` preservation — see below) is delegated to the shared `web/js/pgp/quoted-content.js` module (`formatDecryptedContentAsHtml`/`formatDecryptedContentAsPlainTextHtml`).
...
**Large messages** (when `buildQuotedReplyHtml()` had to truncate — detected by checking its output for the exported `REPLY_TRUNCATION_NOTICE` marker): `handleReplyEncrypted()` instead calls `Office.context.mailbox.item.displayReplyForm()`/`displayReplyAllForm()` with **no custom body**, letting Outlook build its native reply — proper `In-Reply-To`/`References` threading, recipients (self excluded from Reply All automatically), and "Re:" subject, all handled by Outlook itself. Outlook's native quote still shows the original message *as Outlook has it: PGP-armored*, since Outlook has no notion of decryption. `MessageRead.js` then hands the decrypted plaintext to the new compose window over a fixed `BroadcastChannel` (`'pgp_reply_handoff'` — there is no way to control the new window's URL the way the pop-out dialog does, so there's nowhere to embed a per-instance token for it to read on load; the token instead travels inside each broadcast message, to correlate it with its ack), repeating the broadcast every ~400 ms (since a message posted before the compose window's listener is ready is silently dropped, with no queueing for late subscribers) for up to `REPLY_HANDOFF_TIMEOUT_MS` (10 s).

`MessageCompose.js` listens for this handoff unconditionally from module load (a no-op for every ordinary compose window unless a matching broadcast actually arrives). On receipt, it immediately acks, then: reads its own current body via `item.body.getAsync` (already-open compose items write via `Body.setAsync`, which is capped at 1 MB — not the 32 KB `htmlBody` cap — since this is the one Office.js body-write path not bound by the constraint the whole large-message path exists to avoid), locates and removes the PGP armor block via the exported `stripPgpArmorBlock()` (walks a detached DOM the same way `MessageRead.js`'s `extractArmorFromHtml()` does — same `<pre>`/`<br>`/block-element handling — but tracks text-node offsets so the located range can be removed rather than only extracted; robust to the armor being split across sibling nodes, or wrapped in a `<pre>` the way this add-in's own `setBodyAsync()` sends it), splices the formatted decrypted content in at that location, and writes the result back via `item.body.setAsync`. If the armor block can't be found, or anything throws, the body is left untouched and a warning status is shown — no partial edits are attempted.

If no ack arrives within the timeout, `MessageRead.js` falls back to the **normal-sized-message path** above (`displayNewMessageForm` + `buildQuotedReplyHtml`, truncation and all) — this is mechanically unavoidable once `displayReplyForm` has already opened a window, since Office.js gives no way to retract it or get a live handle to it, so the fallback opens a *second* window. A distinct warning is shown *only* in this fallback case, telling the user a backup window was opened and to close the other (still-blank/still-armored) one.
