...
The two reply buttons' DOM ids (`btn-reply-encrypted`, `btn-reply-all-encrypted`, `web/MessageRead.html`) are historical and do not match what they say on screen: a relabel predating this doc's history made `btn-reply-encrypted` the **"Reply All"** button and `btn-reply-all-encrypted` the **"Reply Sender"** button, without renaming either id. Wiring the click handlers' `replyAll` boolean by id name instead of by displayed label has caused Reply and Reply All to be silently swapped twice (most recently as a regression from a "fix" that matched the boolean to the id name). `REPLY_BUTTON_WIRING` (exported from `MessageRead.js`) makes the id→boolean mapping an explicit, tested data structure for exactly this reason — `tests/message-read-reply-button-wiring.test.js` pins it down without needing to invoke `Office.onReady`'s full body.
