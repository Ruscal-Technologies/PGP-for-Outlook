...
| `KeyManagement.html/.js` | Key generation, import, export, contacts keyring, org settings, Ko-fi support button (dynamically injected; suppressed by `hideSupportButton` org config); personal preferences including auto-encrypt/auto-send; links to the online User's Guide | Mailbox 1.1 |
...
Encrypting to your own public key is always done unconditionally alongside recipient/company keys (so the sender can read their own sent mail) — this also means the sender's own private key is always sufficient to reverse an encrypt performed from this add-in's own compose pane, regardless of which recipients were chosen. A "Decrypt" button (`refreshComposeButtons()`/`handleDecrypt()` in `MessageCompose.js`) appears whenever the compose body is currently PGP-armored, letting the user restore the original body and revert any `.pgp` attachments back to their originals so recipients, body, or attachments can be edited before re-encrypting (#25). Attachment reversal is best-effort per file: a `.pgp` attachment that fails to decrypt (e.g. hand-corrupted armor) is left alone and named in a warning status, without blocking the body restore or any other attachment. The decrypted file is added *before* the `.pgp` original is removed (not the reverse) — if `addFileAttachmentFromBase64Async` itself fails (quota, size limit, transient Office error), the original stays attached and the attachment is simply reported as failed, rather than being deleted with nothing to replace it.

### Auto-encrypt and auto-send (Manage PGP → Personal Preferences)

Two opt-in preferences (`pgp_auto_encrypt`, `pgp_auto_send` in `key-storage.js`; both default `false`) let a user skip clicking Encrypt (and, separately, Send) for the common case. Both are gated on a single human action — opening the compose pane's Encrypt view — not on any live per-keystroke event; there is no `RecipientsChanged` listener anywhere in this add-in.

`maybeAutoEncrypt()` (`MessageCompose.js`) runs once, immediately after the pane's initial recipient load, and only if `getAutoEncryptDefault()` is on. If not every recipient already has a resolved key, it shows a waiting status and re-runs `loadRecipients()` every 2 seconds — reusing the existing recipient/key-resolution logic outright rather than duplicating it — for as long as each pass makes forward progress (a new recipient appears, or a previously-keyless one gets a key). It gives up, with a status explaining why, the moment a pass produces the exact same recipient/key-found snapshot as the one before it; this needs no arbitrary invented timeout because `loadRecipients()` already fully awaits both Outlook's own recipient-resolution poll and the complete local→WKD→VKS key-discovery chain before returning.

Once eligible, `handleAutoEncrypt()` pre-checks the two interactive decision points inside `handleEncrypt()` — inline attachments needing the Convert/Continue/Cancel choice, and the attachment-removal confirmation on Mailbox <1.8 hosts — and aborts (leaves the message for manual encryption) if either would be needed, rather than deciding on the user's behalf. Signing is the one exception: if the sign-by-default preference is on and no session key is cached, the passphrase modal pops up automatically, exactly as a manual click would trigger it. `handleEncrypt()` itself is never modified by this feature — `handleAutoEncrypt()` is a thin wrapper that calls it unchanged, at the cost of redundantly (but harmlessly, since both are idempotent) re-running `loadAttachments()`/`reconcileInlineAttachments()` a second time inside `handleEncrypt()`'s own existing steps.

`handleEncrypt()` guards against two concurrent runs via a module-level `_encryptInFlight` flag (set before any `await`, cleared in its `finally`): a manual click on the Encrypt button landing during the narrow window between `handleAutoEncrypt()`'s own pre-check and its call into `handleEncrypt()` no-ops instead of racing a second invocation over the same module-level recipient/attachment state. `loadRecipients()` also resets `_recipientResults` to `[]` when the To/Cc list is currently empty, rather than leaving a stale prior result in place — otherwise a caller checking "does everything in `_recipientResults` have a key" could see a no-longer-current recipient list as still valid.

`handleEncrypt()` returns a `boolean` — `true` only for the specific invocation that actually performed a successful encrypt, `false` for every other exit (skipped by the in-flight guard, cancelled, failed, or the already-encrypted bailout). `handleAutoEncrypt()` gates `maybeAutoSend()` on this return value, never on ambient UI state like `el('status-bar').classList.contains('pgp-alert--success')` — the status bar is shared, module-wide state, so a call skipped by the in-flight guard could otherwise see a *different*, unrelated invocation's leftover success class and incorrectly conclude it succeeded. `handleEncrypt()`'s own recipient re-check also requires a non-empty `_recipientResults` explicitly (`.length === 0 || !...every(...)`), not just `.every(...)` alone — an empty array vacuously passes `.every()`, which would otherwise let a message whose To/Cc emptied out between the Encrypt click and this recheck proceed to encrypt (and, with auto-send on, send) to no external recipient at all, only the sender/company key.

The auto-encrypt wait loop's own retries merge each fresh `loadRecipients()` result with the recipient list as it stood immediately beforehand (`mergePreservingManuallyResolvedKeys()`, exported for testing), preserving any key that already existed before that poll if the fresh pass has none for the same email. This matters because a key the user manually pastes via the recipient list's own "paste key" action lives only in the in-memory `_recipientResults` array until explicitly saved to the keyring — `resolveRecipients()` has no way to know about it, so without this merge the wait loop's own periodic re-polling would silently discard a just-pasted key.

Auto-send (`maybeAutoSend()`) fires only once `handleAutoEncrypt()` sees `handleEncrypt()`'s own return value report a genuine success for *that* call — a *warning* status (e.g. a legacy-algorithm recipient key) does not block it, only a hard failure (or the call being skipped entirely by `_encryptInFlight`) does. It requires `Office.context.mailbox.item.sendAsync()`, which needs **Mailbox 1.15** — a materially newer requirement set than any other capability this add-in gates on — and the manifest's `<Permissions>` element was bumped from `ReadWriteItem` to `ReadWriteMailbox` to allow it (`manifest/manifest.json`, the unified manifest, already declared the equivalent `Mailbox.ReadWrite.User` permission and needed no change). Because roaming settings sync across devices, `maybeAutoSend()` calls `Office.context.requirements.isSetSupported('Mailbox', '1.15')` live at call time — not a cached module-level flag set once in `Office.onReady` (unlike `_has17`/`_has18`/`_has110`/`_has114` elsewhere in this file) — so a preference enabled on a 1.15-capable desktop can't silently fire on an older host the same user later composes from. `KeyManagement.js` is different: it does cache its own `_has115`, set once in its own `Office.onReady`, purely to gate the auto-send preference checkbox's visibility in the Manage PGP pane. The Manage PGP pane's auto-send checkbox is hidden entirely (not shown-but-disabled) wherever `_has115` is false, matching this add-in's existing `_has17`/`_has18`/`_has110`/`_has114` degradation pattern; it's also hidden whenever auto-encrypt itself is off. Saving preferences computes the *final* desired auto-send value up front — `autoEncryptDefault && checkbox.checked` when `_has115` is true (matching what's on screen), or whatever is already stored when it's false — and persists both preferences together via `saveAutoEncryptAndSendDefaults()`, a single atomic write (one `saveAsync()` call). This has to be one write, not two: `saveAutoEncryptDefault(false)` on its own force-clears auto-send as a side effect (correct and intended on a host that can show both checkboxes), so restoring the prior value via a *second*, separate `saveAsync()` call would leave a real gap — if that second save failed (a genuine, already-handled failure mode; every write function in `key-storage.js` propagates a roaming-settings sync/quota failure as a rejection), the first save's force-off would already be persisted remotely, permanently losing the value on a host that can't even render the auto-send toggle to let the user knowingly change it. Per Microsoft's own `sendAsync` docs, code meant to run after a *successful* send isn't guaranteed to execute (the add-in may already be torn down) — `maybeAutoSend()` shows no status before or during sending, matching this; a *failed* send is still reliably caught and shown as an error status.

**Permission-change rollout note:** bumping `<Permissions>` is a real, user/admin-visible escalation — Outlook shows a different consent description for `ReadWriteMailbox` than `ReadWriteItem` — and, consistent with this add-in's experience shipping event-based activation (see "Event-based activation (#22)" below), should be expected to require fresh admin re-approval per tenant before auto-send works anywhere, independent of when the code itself ships.

### Reply / Reply All quoting

The two reply buttons' DOM ids (`btn-reply-all-encrypted`, `btn-reply-sender-encrypted`, `web/MessageRead.html`) were renamed to match their own displayed labels ("Reply All" / "Reply Sender") after the *previous* ids (`btn-reply-encrypted` / `btn-reply-all-encrypted`, which predated a relabel and no longer matched what the buttons said on screen) caused the click handlers' `replyAll` boolean to be silently swapped twice — once when "fixed" for issue #20 by matching the boolean to the misleading old id name instead of the label. `tests/message-read-reply-button-wiring.test.js` asserts the HTML labels and the JS wiring directly against the source text, so a future copy-paste error in either file fails a test instead of shipping silently a third time.
