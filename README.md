...
- The pop-out decrypted-content window (`Open in full window` button) falls
  back to `window.open()` on mobile, which mobile WebViews block.  The
  decrypted content is still readable in the task pane itself.
...
**Test suite**
Implemented: a Vitest suite in `tests/` covering `pgp-core.js` (real crypto round trips — multi-recipient, unicode, empty string, RSA-4096; tampering and wrong-key failure modes; legacy DSA+ElGamal key support via a real fixture key, including `hasWeakEncryptionKey`/`hasModernSubkeys`/`addModernSubkeys`/`extractPublicKey`), `key-storage.js` (including its `saveAsync` failure path), `keyring.js`, `key-discovery.js`, `org-config.js`, `session-cache.js`, and `wkd.js` (`encodeZBase32` plus `lookup()`'s advanced/direct URL fallback and error handling) — no DOM required, run with `npm test` (or `npm run test:coverage` for a v8 coverage report, currently ~97% for these files). Wired into CI (`.github/workflows/deploy-pages.yml`) as a gate before the GitHub Pages deploy; the coverage report is uploaded as a workflow artifact for visibility (not a gate).
Still outstanding: integration tests for the compose/read/key-management task panes using Office mock libraries and a DOM environment (jsdom), and the SHA-1-self-signature legacy-key retry path specifically (narrower than before — the ElGamal/DSA weak-key rejection path is now covered).
